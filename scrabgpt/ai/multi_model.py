...
async def propose_move_multi_model(
    ...
    *,
    ...
    async def call_one_model(model_info: dict[str, Any]) -> dict[str, Any]:
        ...
        max_tokens = model_info.get("max_tokens", 3600)

        log.info("Calling model: %s", model_id)
        ...
        def build_openrouter_meta(resp: dict[str, Any]) -> dict[str, Any]:
            return {
        ...
        openrouter_meta = build_openrouter_meta(result)
        elapsed_value = openrouter_meta.get("elapsed")
        elapsed_display = (
            f"{elapsed_value:.2f}s" if isinstance(elapsed_value, (int, float)) else "?"
        )
        log.info(
            "OpenRouter finished for %s status=%s call_id=%s trace=%s elapsed=%s",
            model_id,
            result.get("status"),
            openrouter_meta.get("call_id", "-"),
            openrouter_meta.get("trace_id", "-"),
            elapsed_display,
        )
        if result.get("error"):
            log.warning(
                "Model %s reported error from OpenRouter: %s", model_id, result.get("error")
            )
        ...
        try:
            ...
            return await _notify({
                ...
                "move": move,
                "score": score,
                "raw_response": raw_content,  # Store raw response for debugging
            ...
            # Return parse error without retry attempts
            raw_response = result.get("content", "")
            
            # Provide helpful analysis of what went wrong
    ...
    if not judge_valid_results:
        ...
        # Fall back to highest-scoring parsed move
        valid_results.sort(key=lambda r: int(r.get("score", -1)), reverse=True)
        best_result = valid_results[0]
    else:
        # Sort by score and select best valid move
