...
from ..ai.juls_agent import agent_validate_word, plain_validate_word, tool_lookup_juls
...
class _LexiconWorker(QObject):
    ...
    def run(self) -> None:
        try:
            result = agent_validate_word(self._word, openai_api_key=self._api_key)
            self.finished.emit(result)
        except Exception as exc:  # noqa: BLE001
            self.failed.emit(exc)


class _PlainLexiconWorker(QObject):
    finished: Signal = Signal(dict)
    failed: Signal = Signal(Exception)
    ...
    def run(self) -> None:
        try:
            result = plain_validate_word(self._word)
            self.finished.emit(result)
        except Exception as exc:  # noqa: BLE001
            self.failed.emit(exc)


class LexiconAgentDialog(QDialog):
    """Manuálne testovanie JÚĽŠ agenta z hlavného toolbaru."""

    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("Agent jazykovedného ústavu")
        self.resize(480, 320)
        ...
        self.word_edit.setPlaceholderText("Zadaj slovo na overenie…")
        row.addWidget(self.word_edit)
        layout.addLayout(row)

        self.verify_btn = QPushButton("Overiť", self)
        layout.addWidget(self.verify_btn, alignment=Qt.AlignmentFlag.AlignLeft)
        ...
        self._worker: _LexiconWorker | None = None
        self._in_progress = False
        self._last_word: str | None = None

        self.verify_btn.clicked.connect(self._on_verify_clicked)
        self.word_edit.returnPressed.connect(self._on_verify_clicked)
    ...
    def _on_verify_clicked(self) -> None:
        ...
        if not word:
            QMessageBox.warning(self, "Agent", "Najprv zadaj slovo na overenie.")
            return
        ...
        if not api_key:
            QMessageBox.warning(
                self,
                "Agent",
                "Nastav OPENAI_API_KEY v nastaveniach pred spustením agenta.",
            )
            return
        self._set_in_progress(True)
        self._last_word = word
        self.reason_label.setText("Vyhľadáva sa v JÚĽŠ…")
        self.quote_label.clear()
        self._start_worker(word, api_key)

    def _start_worker(self, word: str, api_key: str) -> None:
        thread = QThread(self)
        worker = _LexiconWorker(word, api_key)
    ...
    def _on_worker_finished(self, payload: dict[str, object]) -> None:
        ...
        quote = ""
        if self._last_word:
            quote = RefereeDisputeDialog._extract_quote_from_payload(payload, self._last_word)
        self.reason_label.setText(reasoning or "Agent neposkytol odôvodnenie.")
        self.quote_label.setText(quote or "Bez dostupnej citácie.")

    def _on_worker_failed(self, error: Exception) -> None:
        self._set_in_progress(False)
        QMessageBox.critical(self, "Agent", f"Overenie zlyhalo: {error}")

    def _set_in_progress(self, state: bool) -> None:
        self._in_progress = state
        self.verify_btn.setEnabled(not state)
        if state:
            self.verify_btn.setText("Prebieha…")
        else:
            self.verify_btn.setText("Overiť")


class LexiconPlainDialog(QDialog):
    """Manuálne overenie priamo z JÚĽŠ bez LLM."""

    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("Len slovník JÚĽŠ")
        self.resize(480, 320)
        ...
        self._worker_thread: QThread | None = None
        self._worker: _PlainLexiconWorker | None = None
        self._in_progress = False
        self.verify_btn.clicked.connect(self._on_verify_clicked)
        self.word_edit.returnPressed.connect(self._on_verify_clicked)
    ...
    def _on_verify_clicked(self) -> None:
        ...
        word = self.word_edit.text().strip()
        if not word:
            QMessageBox.warning(self, "Slovník", "Najprv zadaj slovo na overenie.")
            return
        self._set_in_progress(True)
        self.reason_label.setText("Vyhľadáva sa v JÚĽŠ…")
        self.quote_label.clear()
        self._start_worker(word)

    def _start_worker(self, word: str) -> None:
        thread = QThread(self)
        worker = _PlainLexiconWorker(word)
        worker.moveToThread(thread)
        thread.started.connect(worker.run)
        worker.finished.connect(thread.quit)
        worker.failed.connect(thread.quit)
    ...
    def _on_worker_finished(self, payload: dict[str, object]) -> None:
        ...
        if not quote:
            quote = RefereeDisputeDialog._extract_quote_from_payload(payload, self.word_edit.text().strip())
            ...
            if isinstance(matches, list):
                for item in matches:
                    if isinstance(item, dict):
                        dict_code = str(item.get("dictionary", "")).strip()
                        if dict_code:
                            dictionaries.append(dict_code)
        valid_msg = (
            "Slovo sa v slovníkoch nenašlo."
            if not dictionaries
            else "Nájdené slovníky: " + ", ".join(dictionaries)
        )
        if payload.get("valid") is False and dictionaries:
            valid_msg += " (bez jednoznačného záznamu)."
        elif payload.get("valid"):
            valid_msg += " — slovo je platné."
        self.reason_label.setText(valid_msg)
        self.quote_label.setText(quote or ("Bez dostupnej citácie." if dictionaries else ""))

    def _on_worker_failed(self, error: Exception) -> None:
        self._set_in_progress(False)
        QMessageBox.critical(self, "Slovník", f"Vyhľadávanie zlyhalo: {error}")

    def _set_in_progress(self, state: bool) -> None:
        self._in_progress = state
        self.verify_btn.setEnabled(not state)
        self.verify_btn.setText("Prebieha…" if state else "Overiť")

class _DisputeWorker(QObject):
    finished: Signal = Signal(dict)
    failed: Signal = Signal(Exception)
    ...
    def run(self) -> None:
        try:
            resp = self._client.dispute_word(
                word=self._word,
...
class RefereeDisputeDialog(QDialog):
    """Chatovací dialog na argumentovanie neplatného slova."""

    @staticmethod
    def _extract_quote_from_payload(
        payload: dict[str, object],
        fallback_word: str,
    ) -> str:
        direct = payload.get("quote")
        ...
        try:
            proof = tool_lookup_juls(fallback_word)
        except Exception:
            return ""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(parent)
        self.setWindowTitle("Argumentácia s rozhodcom")
        self.resize(520, 400)

        self._client = client
        self._word = word
        self._language = language
        self._history: list[tuple[str, str]] = [("referee", initial_reason)]
        self._attempts_used = 0
        self._max_attempts = 3
        self._waiting = False
        self._finalized = False
        self._worker_thread: QThread | None = None
        self._worker: _DisputeWorker | None = None
        self._lookup_thread: QThread | None = None
        self._lookup_worker: _LexiconWorker | None = None
        self._lookup_in_progress = False
        self._lookup_available = self._language.lower() == "slovak"

        self.result_valid = False
        self.result_reason = initial_reason
        self._last_reported_attempts_left: int | None = None
        ...
        self.input_edit = QLineEdit(bottom)
        self.input_edit.setPlaceholderText(
            "Napíš argument, prečo je slovo platné…"
        )
        bottom_layout.addWidget(self.input_edit, 1)
        self.send_btn = QPushButton("Odoslať", bottom)
        self.send_btn.setDefault(True)
        self.lookup_btn = QPushButton("Vyhľadať v Jazykovednom ústave", bottom)
        self.lookup_btn.setMinimumWidth(220)
        bottom_layout.addWidget(self.send_btn)
        bottom_layout.addWidget(self.lookup_btn)
        layout.addWidget(bottom)
        self._input_container = bottom
        ...
        self._append_message("Rozhodca", initial_reason)
        self._update_hint()
        self._update_lookup_button_state()

    def _append_message(self, speaker: str, text: str) -> None:
        safe = html.escape(text.strip() or "(bez odôvodnenia)")
        safe = safe.replace("\n", "<br>")
        self.chat_view.append(f"<p><b>{speaker}:</b> {safe}</p>")
        try:
    ...
    def _on_send_clicked(self) -> None:
        ...
        if not text:
            return
        self._submit_argument(text)

    def _submit_argument(self, text: str) -> None:
        self.input_edit.clear()
        self._append_message("Hráč", text)
        self._history.append(("player", text))
        next_attempt = self._attempts_used + 1
        attempts_left = max(0, self._max_attempts - next_attempt)
        self._transition_waiting(True)
        self._start_worker(list(self._history), attempts_left)

    def _start_worker(self, history: list[tuple[str, str]], attempts_left: int) -> None:
        thread = QThread(self)
        worker = _DisputeWorker(
            self._client,
            self._word,
            self._language,
            history,
    ...
    def _on_worker_finished(self, result: dict[str, object]) -> None:
        ...
        self._transition_waiting(False)

        reason = str(result.get("reason", "")).strip()
        if not reason:
            reason = "Rozhodca neposkytol ďalšie odôvodnenie."
        self.result_reason = reason
        self.result_valid = bool(result.get("valid", False))
        self._history.append(("referee", reason))
        self._append_message("Rozhodca", reason)

        reported_left = result.get("attempts_left")
        if isinstance(reported_left, int) and 0 <= reported_left <= self._max_attempts:
            self._last_reported_attempts_left = reported_left
            self._attempts_used = max(0, min(self._max_attempts, self._max_attempts - reported_left))
        else:
            self._attempts_used = max(0, min(self._max_attempts, self._attempts_used + 1))

        if self.result_valid or self._attempts_used >= self._max_attempts:
            self._enter_final_state()
        else:
            self._update_hint()
        self._update_lookup_button_state()
    ...
    def _on_worker_failed(self, error: Exception) -> None:
        ...
        QMessageBox.critical(
            self,
            "Argumentácia",
            f"Rozhodcu sa nepodarilo kontaktovať: {error}",
        )
        self._update_hint()
        self._update_lookup_button_state()

    def _transition_waiting(self, waiting: bool) -> None:
        self._waiting = waiting
        if waiting:
            self.input_edit.setEnabled(False)
            self.send_btn.setEnabled(False)
            self.send_btn.setText("Čaká sa…")
            self.meta_label.setText("Rozhodca vyhodnocuje argument…")
        else:
            if not self._finalized:
                self.input_edit.setEnabled(True)
                self.send_btn.setEnabled(True)
                self.send_btn.setText("Odoslať")
        self._update_lookup_button_state()

    def _enter_final_state(self) -> None:
        self._finalized = True
        self.input_edit.setVisible(False)
        self.send_btn.setEnabled(True)
        self.send_btn.setText("OK")
        self.send_btn.setDefault(True)
        if self._lookup_available:
            self.lookup_btn.setVisible(False)
        self._update_hint()
        self._update_lookup_button_state()
    ...
    def _update_hint(self) -> None:
        if self._finalized:
            if self.result_valid:
                self.meta_label.setText(
                    "Rozhodca nakoniec slovo uznal. Pokračuj stlačením OK."
                )
            else:
                self.meta_label.setText(
                    "Rozhodca definitívne slovo odmietol. Pokračuj stlačením OK."
                )
            return
        ...
        if remaining <= 0:
            self.meta_label.setText(
                "Vyčerpali ste všetky pokusy. Počkajte na rozhodcu."
            )
            return
        if remaining == 1:
            self.meta_label.setText(
                "Ostáva ešte 1 pokus na presvedčenie rozhodcu."
            )
        elif remaining in {2, 3, 4}:
            self.meta_label.setText(
                f"Ostávajú ešte {remaining} pokusy na presvedčenie rozhodcu."
            )
        else:
            self.meta_label.setText(
                f"Ostáva ešte {remaining} pokusov na presvedčenie rozhodcu."
            )

    def reject(self) -> None:
        if self._waiting and not self._finalized:
            return
        super().reject()
    ...
    def _on_lookup_clicked(self) -> None:
        if (
            self._finalized
            or self._waiting
            or self._lookup_in_progress
            or not self.lookup_btn.isVisible()
        ):
            return
        if not self._lookup_available:
            return
        ...
        if not api_key:
            QMessageBox.warning(
                self,
                "Vyhľadávanie v JÚĽŠ",
                "Najprv nastav OPENAI_API_KEY v nastaveniach.",
            )
            return
        self._lookup_in_progress = True
        self._update_lookup_button_state()
        self.meta_label.setText("Vyhľadáva sa v JÚĽŠ…")
        self._start_lookup_worker(api_key)

    def _start_lookup_worker(self, api_key: str) -> None:
        thread = QThread(self)
        worker = _LexiconWorker(self._word, api_key)
        worker.moveToThread(thread)
        thread.started.connect(worker.run)
        worker.finished.connect(thread.quit)
        worker.failed.connect(thread.quit)
    ...
    def _on_lookup_finished(self, payload: dict[str, object]) -> None:
        self._lookup_in_progress = False
        reasoning = str(payload.get("reasoning", "")).strip()
        quote = self._extract_quote_from_payload(payload, self._word)
        argument_parts: list[str] = []
        if reasoning:
            argument_parts.append(reasoning)
        if quote:
            argument_parts.append(f"Citácia: {quote}")
        if not argument_parts:
            argument_parts.append("Agent JÚĽŠ nenašiel použiteľný dôkaz.")
        argument_text = "\n".join(argument_parts)
        self.meta_label.setText("Argument od agenta bol odoslaný rozhodcovi.")
        self._update_lookup_button_state()
        self._submit_argument(argument_text)
        self.lookup_btn.setVisible(False)

    def _on_lookup_failed(self, error: Exception) -> None:
        self._lookup_in_progress = False
        self._update_lookup_button_state()
        QMessageBox.critical(
            self,
            "Vyhľadávanie v JÚĽŠ",
            f"Vyhľadávanie zlyhalo: {error}",
        )
        self._update_hint()
        self.lookup_btn.setVisible(False)

    def _update_lookup_button_state(self) -> None:
        if not self._lookup_available:
            return
        if self._finalized:
            self.lookup_btn.setVisible(False)
        ...
        if self._lookup_in_progress:
            ...
            self.lookup_btn.setText("Vyhľadáva sa…")
            return
        if self._waiting:
            self.lookup_btn.setEnabled(False)
            self.lookup_btn.setText("Vyhľadať v Jazykovednom ústave")
            return
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.act_juls_agent = QAction("Agent jazykovedného ústavu", self)
        self.act_juls_agent.triggered.connect(self.open_juls_agent_dialog)
        self.toolbar.addAction(self.act_juls_agent)

        self.act_juls_plain = QAction("Len slovník bez LLM", self)
        self.act_juls_plain.triggered.connect(self.open_plain_juls_dialog)
        self.toolbar.addAction(self.act_juls_plain)
    ...
    def open_juls_agent_dialog(self) -> None:
        dlg = LexiconAgentDialog(self)
        dlg.exec()

    def open_plain_juls_dialog(self) -> None:
        dlg = LexiconPlainDialog(self)
        dlg.exec()
    ...
    def _on_judge_ok(self, resp: dict[str, object]) -> None:
        ...
        if not all_valid:
            ...
            if invalid_entries:
                ...
                # rozhodca argumentácia
                if self.ai_client is None:
                    self.ai_client = OpenAIClient()
                dispute_dialog = RefereeDisputeDialog(
                    self,
                    self.ai_client,
                    word=bad_word,
                    initial_reason=bad_reason,
                    language=self.variant_language,
                )
                dispute_dialog.exec()
                final_reason = dispute_dialog.result_reason or bad_reason
                primary["reason"] = final_reason
                if dispute_dialog.result_valid:
                    primary["valid"] = True
                    all_valid = all(bool(item.get("valid", False)) for item in entries)
                    if not all_valid:
                        next_invalid = next(
                            (item for item in entries if not bool(item.get("valid", False))),
                            primary,
                        )
                        next_word = str(next_invalid.get("word", bad_word))
                        next_reason = str(next_invalid.get("reason", final_reason))
                        self._handle_invalid_turn(next_word, next_reason)
                        return
                else:
                    self._handle_invalid_turn(bad_word, final_reason)
                    return
            else:
